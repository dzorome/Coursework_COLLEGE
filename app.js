...
function checkAdmin(req, res, next) {
    if (req.session.role === 'staff') {
        ...
        res.status(403).send('Доступ запрещен, ня');
    }
}
...
app.get('/admin', checkAuth, checkAdmin, (req, res) => {
    db.all('SELECT * FROM employees', [], (err, employees) => {
        ...
        db.all("SELECT * FROM prescriptions", (err, prescriptions) => {
            ...
            // Показ в админ-панели сотрудников и рецептов
            res.render('admin', { employees: employees, prescriptions: prescriptions });
        });
    });
});

// Маршрут для отображения рецептов
app.get('/admin/prescriptions', checkAuth, (req, res) => {
    db.all('SELECT * FROM prescriptions', [], (err, prescriptions) => {
...
// Фильтрация по сотрудникам или пациентам
app.get('/admin/filter', checkAuth, (req, res) => {
    ...
    db.all(query, params, (err, results) => {
        if (err) return res.status(500).send('Ошибка базы данных');
        res.render('admin', { employees: results });
    });
});

// Экспорт записей
app.get('/admin/export', checkAuth, (req, res) => {
    db.all('SELECT * FROM employees', [], (err, employees) => {
        if (err) return res.status(500).send('Ошибка базы данных');
...
app.post('/admin/add-client', checkAuth, checkAdmin, (req, res) => {
